...
def create_app():
    ...
    def unauthorized_callback(callback):
        app.logger.warning(f"⚠️ JWT unauthorized: {callback}")
        return jsonify({
    ...
    def invalid_token_callback(callback):
        app.logger.warning(f"⚠️ JWT invalid token: {callback}")
        return jsonify({
            "error": "Invalid token",
    ...
    # IMPORTANT: Initialiser et démarrer le scheduler APRÈS la DB et les blueprints
    try:
        # Initialiser le scheduler avec l'app
        scheduler.init_app(app)
        app.logger.info("✅ Scheduler initialisé")
        
        # Démarrer le scheduler dans le contexte de l'app
        with app.app_context():
            scheduler.start()
    ...
    # ============ SHUTDOWN HANDLERS ============
    # Enregistrer UNE SEULE FOIS le shutdown
    ...
    # Enregistrer le shutdown avec atexit (appelé à la fermeture de l'app)
    atexit.register(shutdown_scheduler)
    
    # Enregistrer également avec teardown_appcontext (appelé à chaque fin de contexte)
    @app.teardown_appcontext
    def teardown_scheduler(exception=None):
        """Cleanup du scheduler en cas d'erreur dans le contexte"""
        if exception:
            app.logger.error(f"⚠️ Exception dans le contexte: {exception}")
        # Ne pas appeler shutdown ici car déjà géré par atexit
